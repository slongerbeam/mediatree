...
capture_remove_next_buf(struct imx_media_video_dev *vdev, void *unused,
			enum v4l2_buf_type type)
{
	...
	/* get next queued buffer and remove from queue */
	...
	return buf ? &buf->vbuf : NULL;
}

static void capture_buf_done(struct imx_media_video_dev *vdev, void *unused,
			     enum v4l2_buf_type type,
...
static const struct imx_media_video_dev_ops capture_ops = {
	...
	.remove_next_buf = capture_remove_next_buf,
...
int imx_media_capture_device_register(struct imx_media_video_dev *vdev)
{
	...
	priv->cc = imx_media_find_pixel_format(priv->fmt.fmt.pix.pixelformat,
					       PIXFMT_SEL_ANY);
