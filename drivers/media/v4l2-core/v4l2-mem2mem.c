 ...
 *			Used only when the M2M device is registered via
 *			v4l2_m2m_register_media_controller().
 * @fake_proc:		&struct media_entity pointer with the M2M device itself.
 *			Used only when the M2M device is registered via
 *			v4l2_m2m_register_media_controller() without providing
 *			a real processing entity.
 * @fake_proc_pads:	&struct media_pad with the @proc pads.
 *			Used only when the M2M device is registered via
 *			v4l2_m2m_register_media_controller() without providing
 *			a real processing entity.
...
struct v4l2_m2m_dev {
	...
	struct media_entity	fake_proc;
	struct media_pad	fake_proc_pads[2];
...
void v4l2_m2m_unregister_media_controller(struct v4l2_m2m_dev *m2m_dev)
{
	...
	if (m2m_dev->proc_sink->entity == &m2m_dev->fake_proc)
		media_entity_remove_links(&m2m_dev->fake_proc);
	media_device_unregister_entity(m2m_dev->source);
	media_device_unregister_entity(&m2m_dev->sink);
	if (m2m_dev->proc_sink->entity == &m2m_dev->fake_proc)
		media_device_unregister_entity(&m2m_dev->fake_proc);
	kfree(m2m_dev->source->name);
	kfree(m2m_dev->sink.name);
	if (m2m_dev->proc_sink->entity == &m2m_dev->fake_proc)
		kfree(m2m_dev->fake_proc.name);
}
	...
	struct video_device *vdev, int function)
{
	...
	case MEM2MEM_ENT_TYPE_PROC:
		entity = &m2m_dev->fake_proc;
		pads = m2m_dev->fake_proc_pads;
		pads[0].flags = MEDIA_PAD_FL_SINK;
...
err_rm_links1:
	media_entity_remove_links(&m2m_dev->sink);
err_rm_links0:
	if (proc_sink->entity == &m2m_dev->fake_proc)
		media_entity_remove_links(&m2m_dev->fake_proc);
	media_entity_remove_links(m2m_dev->source);
err_rel_entity1:
	media_device_unregister_entity(&m2m_dev->sink);
		...
		struct video_device *vdev, int function)
{
	...
	ret = __v4l2_m2m_register_media_controller(m2m_dev, vdev,
			&m2m_dev->fake_proc_pads[0],
			&m2m_dev->fake_proc_pads[1],
			MEDIA_LNK_FL_IMMUTABLE | MEDIA_LNK_FL_ENABLED,
			MEDIA_LNK_FL_IMMUTABLE | MEDIA_LNK_FL_ENABLED);
	if (ret)
		goto err_rel_entity;
